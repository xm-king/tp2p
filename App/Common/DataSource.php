...
function get_personal_benefit($uid)
{
    ...
    // 统计流转标回款利息interest、回款总额capital、利息手续费fee
    $transfer_investor =  M("transfer_investor_detail")
                            ->field("sum(receive_capital) as capital, sum(receive_interest) as interest, sum('interest_fee') as fee")
                            ->where("investor_uid={$uid} and status = 1 and pay_status=1")->find();
    ...
    $transfer_interest_collection = M('transfer_investor_detail')
                                    ...
                                    ->where("investor_uid={$uid} and status = 7 and pay_status=1")
                                    ->find();  
    
    $benefit['interest_collection'] =  $interest_collection['interest'] + $transfer_interest_collection['interest'] -$transfer_interest_collection['fee'];//代收利息 
	$benefit['capital_collection'] =  $interest_collection['capital'] + $transfer_interest_collection['capital'] ; // 代收本金
...
function get_personal_out($uid)
{
    ...
    $interest =  M("investor_detail")
                            ...
                            ->where("borrow_uid={$uid} and status in (1,2,3,4,5) and pay_atatus=1")
							->find();                     
   $t_interest =  M("transfer_investor_detail")
                            ->field('sum(receive_capital) as capital, sum(receive_interest) as interest,sum(interest_fee) as interest_fee')
                            ->where("borrow_uid={$uid} and status =1 and pay_status=1")
                            ->find();                     
    $out['interest'] = $interest['interest'] + $t_interest['interest'];   //支付利息 
	$out['capital'] = $interest['capital'] + $t_interest['capital']; // 已还本金
    ...
    $interest_pay = M('investor_detail')
                            ...
                            ->find();
    $t_interest_pay = M('transfer_investor_detail')
                            ->field('sum(interest) as interest, sum(capital) as capital')
                            ->where("borrow_uid={$uid} and status = 7 and pay_status=1")
                            ->find();
    $out['interest_pay'] =  $interest_pay['interest'] + $t_interest_pay['interest']; //待还利息
    $out['capital_pay'] = $interest_pay['capital'] + $t_interest_pay['capital']; //待还金额
...
function get_personal_count($uid)
{
    ...
    $t_ljtz = M('transfer_borrow_investor')->where("investor_uid={$uid} and loanno<>''")->sum('investor_capital');
    $count['ljtz'] = $p_ljtz + $t_ljtz;
    //**************
    //累计借入金额
    $p_jrje = M('borrow_info')->where("borrow_uid={$uid} and borrow_status in (6,7,8,9,10)")->sum('borrow_money');
    $t_jrje = M('transfer_borrow_info')->where("borrow_uid={$uid}")->sum('transfer_out*per_transfer');
    $count['jrje'] = $p_jrje + $t_jrje;
    //****************
    //*****累计充值金额***
    ...
    $transfer_interest_fee = M('transfer_investor_detail')->where('investor_uid='.$uid.' and status =1 and  pay_status=1' )->sum('interest_fee');  //流转标投资管理费（统计还款后的管理费）
...
function autoInvest($borrow_id){
    ...
    foreach($autolist as $key=>$v){
        ...
        if($invest_id){
            ...
            $loanList[] = $loan->loanJsonList($invest_qdd['qdd_marked'], $borrow_qdd['qdd_marked'], $orders, $borrow_id, $investMoney, $binfo['borrow_money'],'投标',"对{$borrow_id}号（自动）投标",$secodary);
...
{
    $loan_no_list = '';
    $list = M("_borrow_investor")->field('loanno')->where("borrow_id={$borrow_id} and loanno<>''")->select();
    if(count($list)){
        foreach($list as $v){
