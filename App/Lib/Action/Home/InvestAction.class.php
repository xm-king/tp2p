    ...
    public function notify()
    {
        ...
        if($loan->loanVerify($_POST)){
            ...
            if(intval($_POST['ResultCode']) != 88){
                $this->investRollback($invest_id); //返回错误 删除投资信息
            }else{ //返回成功，更新投资信息并扣除资金
                if($_POST['Action']==2){ // 退回投标
                   $iinfo = M('borrow_investor')->field('investor_capital, investor_uid, borrow_uid')->where("id={$invest_id}")->find();
                   ...
                   // 更新借款状态
                   $borror_info = M('borrow_info')->field('borrow_status, has_borrow, borrow_times')->where("id={$invest_info['BatchNo']}")->find();
                   if($borrow_info['borrow_status']==4){
                        $borrow_x['borrow_status'] = 2;
                   }
                   $borrow_x['borrow_times'] = $borror_info['borrow_times']-1;
                   $borrow_x['has_borrow'] = $borror_info['has_borrow'] - $iinfo['investor_capital'];
                   ...
                   $borrow_res = M('borrow_info')->where("id={$invest_info['BatchNo']}")->save($borrow_x);
                ...
                */
                $pre = C('DB_PREFIX');
                $borrow_id = $invest_info['BatchNo'];
                $money =  $invest_info['Amount'];
                $binfo = M("borrow_info")->field('borrow_money, borrow_type, has_borrow, borrow_uid')->where("id={$borrow_id}")->find();
                $iinfo = M('borrow_investor')->field('investor_uid, loanno')->where("id={$invest_id}")->find();
                if(!$iinfo['loanno']){
					...
					$money_res = memberMoneyLog($iinfo['investor_uid'],6,-$money,"对{$borrow_id}号标进行投标",$binfo['borrow_uid']);
					if($investor_status && $upborrow_res && $detail_status && $money_res ){
						$investMoney->commit(); 
				...
				notifyMsg('普通投标',$_POST, 'http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'], $str);
				echo $str;exit;
            }
            
            notifyMsg('普通投标',$_POST, 'http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'], ''); 
        }
